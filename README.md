...
- 每轮招聘流程支持一键导出 PDF，包含公司、岗位、轮次、时间、题目、回答、参考答案和整体复盘。
...
- 第一份上传的简历会自动成为默认简历，也可在“我的简历”中随时切换；删除默认简历后，系统会自动选取最近更新的一份。
- 新增投递时若“简历版本”和“AI 分析所用简历”均未指定，系统会自动使用默认简历；手动选择始终优先。
...
- 在“数据与备份”中统一配置 AI 提供商、模型、接口地址和 API Key。
- 当前支持 OpenAI Responses API，以及使用 Chat Completions 的 OpenAI 兼容接口。
- 默认模型为 `gpt-5.6-terra`，后续的 JD 解析、简历匹配和面试复盘可以复用同一模型层。
- 在新增或编辑投递时，可选择简历库中的 PDF 或 DOCX，点击“AI 分析岗位”；若没有选择且“简历版本”为空，则自动使用默认简历。系统会读取简历文字并与 JD 对照，生成岗位概述、匹配判断、核心技能、主要职责、简历建议、面试方向和准备清单，结果随投递记录保存在本机。旧版 `.doc` 请先另存为 PDF 或 DOCX。
- 独立的“AI 助手”支持多轮对话，可关联某个岗位和简历，读取对应的 JD、投递状态、招聘流程、面试题目与复盘记录后继续追问。
- 内置岗位匹配、模拟面试、面试复盘和今日计划快捷提问；每条回答会标注实际参考的本地记录。
- AI 会话与消息历史保存在本机 SQLite，可新建、重命名和删除；AI 仅提供建议，不会自动修改投递或日程数据。
- 提供连接测试；API Key 保存后不会由接口返回给网页。
- AI 配置仅保存在本机数据目录，不会提交到 GitHub。
...
│  └─ ai-config.json          # 本机 AI 模型配置（含 API Key）
├─ resumes/                    # 简历文件
├─ attachments/                # 笔试、面试附件
└─ backups/                    # 数据库备份
```

本项目会保存真实求职信息。提交代码到 GitHub 前，请勿把自己的数据库、简历、附件、AI 配置或备份加入仓库。`ai-config.json` 含有明文 API Key，应只留在自己的电脑中。
...
- 创建带日期的 SQLite 数据库备份。
- 导出投递、流程、题目、日程和 AI 对话历史等结构化记录为 JSON。
- 将 JSON 记录合并导入现有数据库。
...
├─ server/
│  ├─ ai/
│  │  └─ model-client.mjs      # 统一模型接口与提供商适配器
